...
const generatorSchema = z.object({
  ...
  outputType: z
    .enum(["Cover Letter", "Resume Bullets", "LinkedIn Summary"])
    .default("Cover Letter"),
  length: z.enum(["short", "medium", "long"]).default("medium"),
  fullDescription: z
    .string()
    .min(10, "Job description is required to tailor the content"),
});
...
function GeneratorFormContent() {
  ...
  const {
    ...
    resolver: zodResolver(generatorSchema as never),
    defaultValues: {
      tone: "confident",
        ...
        if (res.ok) {
          ...
          if (jobIdParam) {
            const foundApp = data.find(
              (app: Application) => app.job?._id === jobIdParam,
            );
            if (foundApp) {
  ...
  async function onSubmit(data: GeneratorForm) {
    ...
    try {
      ...
      // Stream the reader chunks — toTextStreamResponse() yields raw text
  ...
  async function saveToApplication() {
    ...
    try {
      ...
      toast.success("Content saved successfully to your application tracker!");
    } catch (error: unknown) {
  ...
  return (
    <div className="min-h-screen bg-paper-50 py-12">
      <div className="mx-auto max-w-6xl px-6">
        <h1 className="text-3xl font-semibold text-paper-900 mb-2">
          AI Content Generator
        </h1>
        <p className="text-sm text-paper-500 mb-8">
          Generate highly tailored cover letters, resume bullet points, or
          LinkedIn summaries.
        </p>
        ...
        <div className="grid gap-8 lg:grid-cols-2 items-start">
          ...
          <div className="rounded-xl border border-paper-100 bg-paper-0 p-8 shadow-sm">
            <div className="mb-6">
              ...
              <select
                ...
                className={selectCls}>
                <option value="">
                  -- Choose an application to auto-fill --
                </option>
          ...
          {/* Right panel: Output */}
          <div className="rounded-xl border border-paper-100 bg-paper-0 p-8 shadow-sm flex flex-col h-full min-h-125 justify-between">
            <div>
              ...
              {generatedText ? (
                <textarea
                  ...
                  className="w-full min-h-87.5 border-0 focus:outline-none focus:ring-0 text-paper-900 bg-transparent font-sans text-base leading-relaxed resize-y whitespace-pre-wrap"
                />
              ) : (
                <div className="flex flex-col items-center justify-center py-24 text-center text-paper-500">
                  <svg
                    className="h-10 w-10 text-paper-300 mb-2"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={1.5}>
                  ...
                  <p className="text-sm">
                    Tailored response will display here after you hit generate.
                  </p>
                </div>
              )}
            </div>
